...
function placeTheNumber(rowI, colI, generatedNumber) {
    document.getElementById('i' + rowI + colI).value = generatedNumber; // setAttribute('value', generatedNumber); //Writes on HTML
...
function removeCells() {
    ...
    for (let i = difficultyHide; i > 0; i--) {
        ...
        document.getElementById('i' + rowI + colI).value = ""; //setAttribute('value', ''); //Writes on HTML
...
function correctRow(row, checkedSudoku) {
    ...
    let tempArray = [];
    for (let i = 0; i < 9; i++) {
        tempArray[i] = checkedSudoku[row * 9 + i];
    }
    tempArray.sort();
...
function correctCol(col, checkedSudoku) {
    ...
    let tempArray = [];
    for (let i = 0; i < 9; i++) {
        tempArray[i] = checkedSudoku[col + i * 9];
    }
    tempArray.sort();
...
function correctBlock(block, checkedSudoku) {
    ...
    let tempArray = [];
    for (let i = 0; i < 9; i++) {
        tempArray[i] = checkedSudoku[Math.floor(block / 3) * 27 + i % 3 + 9 * Math.floor(i / 3) + 3 * (block % 3)];
    }
    tempArray.sort();
    return tempArray.join() == validArray.join();
}
